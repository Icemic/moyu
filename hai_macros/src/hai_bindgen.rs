...
pub fn entry(args: TokenStream, func_body: TokenStream) -> TokenStream {
    ...
    let head = quote! {
        pub fn #fn_name(__scope: &mut HandleScope, __args: Local<Array>, mut __ret: ReturnValue)
    };
    ...
    let body = quote! {
        ...
        let ret = #fn_name_inner(#(#variable_names,)*);

        let ret = match ret {
            Ok(v) => v,
            Err(msg) => {
                throw_exception!(__scope, msg);
                return;
            }
        };
