...
import { createRoot, animated, useSpring, useScroll } from '../..';
...
人人有资格享有本宣言所载的一切权利和自由，不分种族、肤色、性别、语言、宗教、政治或其他见解、国籍或社会出身、财产、出生或其他身分等任何区别。并且不得因一人所属的国家或领土的政治的、行政的或者国际的地位之不同而有所区别，无论该领土是独立领土、托管领土、非自治领土或者处于其他任何主权受限制的情况之下。`;
  ...
  // makes it loop infinitely between the two states
  const handleClick = () => {
    console.log('click');
    api.start({
      anchor: [1, 1],
  ...
  return (
    <container x={50} y={100}>
      <sprite label="背景图" src={src} rotation={0.2} onClick={() => setSrc('button_n_02.png')} />
      <animated.sprite label="测试图2" src="button_n_02.png" onClick={handleClick} {...springs} y={200} />
      {src === 'title.png' && <sprite label="测试图" src="button_n_02.png" x={300} y={400} />}
