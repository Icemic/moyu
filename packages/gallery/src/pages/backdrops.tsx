...
function ConsolePanel({
  ...
  return (
    <Panel title={t`控制台`} width={400} height={760} note={t`滑块实时调整右侧场景的滤镜参数。`}>
      <vbox gap={24}>
        <text {...GROUP_LABEL} text={t`开关 Toggle`} />
        <Button
          sprite={{ ...BUTTON_SPRITE, targetWidth: 320, targetHeight: 56 }}
          text={blurred ? t`禁用背景滤镜` : t`启用背景滤镜`}
          textStyle={BUTTON_TEXT_STYLE}
          onPress={onToggle}
        />
        <text {...GROUP_LABEL} text={t`模糊半径 Blur`} />
        <Slider
          value={blurValue}
          ...
          thumb={SLIDER_THUMB}
        />
        <text {...TEXT.body} text={t`模糊半径 ${blurRadius}`} />
        <text {...GROUP_LABEL} text={t`饱和度 Saturation`} />
        <Slider
          value={saturation}
          ...
          thumb={SLIDER_THUMB}
        />
        <text {...TEXT.body} text={t`饱和度 ${saturation.toFixed(2)}`} />
        <text
          {...TEXT.caption}
          text={t`Backdrop 捕获其身后已绘制的画面并应用滤镜；在它之后绘制的内容不受影响。`}
          boxWidth={360}
          lineHeight={28}
        />
      </vbox>
...
export function BackdropsPage() {
  ...
  return (
    <container>
      ...
      <Panel title={t`演示场景`} width={1072} height={760} x={432} note={t`模糊与饱和度实时作用于捕获画面。`}>
        ...
        <text text={t`渲染管线`} fontSize={64} fillColor={ITEM_COLORS[1]} x={180} y={340} rotation={0.03} />
        <text text={t`背景捕获区域`} fontSize={70} fillColor={COLOR.text} x={400} y={560} rotation={-0.04} />
        ...
        <vbox x={200} y={360} gap={18}>
          <text text={t`backdrop 之后绘制的内容保持清晰`} fontSize={28} fillColor={COLOR.pageTitle} />
          <DemoChip label={t`前景内容`} width={160} height={48} color={ITEM_COLORS[4]} />
        </vbox>
      </Panel>
    </container>
  );
