...
function SampleContent() {
  return (
    <>
      <sprite src="images/sample.png" targetWidth={248} targetHeight={165} x={20} y={56} />
      <text text="滤镜文字 Aa123" fontSize={22} fillColor={COLOR.text} x={20} y={234} />
      <text text="第二行 Second line" fontSize={18} fillColor={COLOR.caption} x={20} y={266} />
    </>
  );
}
...
function FilterCell({ title, note, filters }: { title: string; note: string; filters?: Filters }) {
  ...
  return (
    <Panel title={title} width={288} height={420} note={note}>
      {content}
    </Panel>
  );
}
  ...
  return (
    <container>
      {rows.map((row, rowIndex) => (
        <hbox key={rowIndex} gap={16} y={rowIndex * 436}>
          {row.map((sample) => (
