    ...
    <container>
      ...
      >
        <vbox padding={18} gap={14}>
          ...
          <text text={t`固有尺寸文本项`} fontSize={26} fillColor={COLOR.text} />
      ...
      <Panel
        ...
        height={400}
        x={512}
        note={t`预期：垂直居中；paddingX/Y 覆盖 padding；所有直接子项参与测量。`}
      >
        <hbox x={20} y={80} padding={8} paddingX={24} paddingY={16} gap={18} alignItems="center">
          ...
          <text text={t`文字固有尺寸`} fontSize={24} fillColor={COLOR.text} />
      ...
      <Panel
        title={t`3. 嵌套：VBox → HBox → VBox`}
        width={992}
        height={416}
        x={512}
        y={432}
        note={t`预期：嵌套尺寸逐层传递；内层两项右对齐；兄弟间无重叠。`}
      >
        <vbox x={20} y={64} gap={16} padding={12}>
          <DemoChip label={t`外层 VBox`} width={280} height={48} color={PURPLE} />
          <hbox gap={20} alignItems="center">
            <DemoChip label={t`左`} width={150} height={78} color={BLUE} />
            <vbox gap={10} paddingX={14} paddingY={8} alignItems="end">
              <DemoChip label={t`内 VBox A`} width={210} height={44} color={GREEN} />
              <DemoChip label={t`内 VBox B`} width={150} height={58} color={RED} />
            </vbox>
            <DemoChip label={t`右`} width={190} height={62} color={ORANGE} />
          </hbox>
        </vbox>
      </Panel>
    </container>
  ...
  return (
    <container>
      <hbox gap={16} alignItems="center">
        ...
        />
        <text {...TEXT.caption} text={t`点击按钮循环切换；两块区域同步变化。`} />
      </hbox>

      <Panel title={t`4. 固定 VBox：520 × 560`} width={600} height={760} y={72} note={t`深色区域为容器的固定尺寸。`}>
      ...
      <Panel
        title={t`5. 固定 HBox：780 × 420`}
        width={872}
        height={760}
        x={632}
        y={72}
        note={t`space-between：间隔相等且不小于 gap = 20。`}
      >
...
function TransformTab() {
  const { t } = useLingui();
  const [measured, setMeasured] = useState(t`等待测量`);

  return (
    <container>
      <Panel
        title={t`6. 直接子项的 x/y 是视觉偏移`}
        width={1504}
        height={380}
        note={t`预期：x = 45 的项目向右覆盖部分间隔，但兄弟项仍按原占位排列；anchor 被布局忽略。`}
      >
        <hbox x={20} y={100} gap={24} alignItems="center">
          <DemoChip label={t`基准 A`} width={190} height={70} color={BLUE} />
          <container x={45}>
            <DemoChip label="x = 45" width={190} height={70} color={GREEN} />
          </container>
          <DemoChip label={t`基准 B`} width={190} height={70} color={RED} />
          <container anchor={[0.5, 0.5]}>
            <DemoChip label="anchor = .5" width={190} height={70} color={ORANGE} />
          </container>
          <DemoChip label={t`基准 C`} width={190} height={70} color={PURPLE} />
        </hbox>
      </Panel>
      ...
      <Panel
        ...
        height={436}
        y={412}
        note={t`预期：变换项按未变换尺寸占位；pivot 右对齐文字不扩大容器测量宽度。`}
      >
        <hbox x={20} y={90} gap={36} alignItems="center">
          <DemoChip label={t`正常`} width={170} height={80} color={BLUE} />
      ...
      <Panel
        title={t`8. onLayout 测量回读`}
        width={732}
        height={436}
        x={772}
        y={412}
        note={t`预期：onLayout 报告的宽度 = 三个色块 + 两个间隔。`}
      >
        <hbox
          x={20}
          y={90}
        ...
        <text {...TEXT.body} text={t`HBox onLayout：${measured}`} x={20} y={230} />
        <text {...TEXT.caption} text={t`理论值：150 + 220 + 180 + 2 × 18 = 586`} x={20} y={270} />
      </Panel>
    </container>
  );
}
...
function DynamicTab() {
  const { t } = useLingui();
  const dynamicLabels = [t`初始布局`, t`B 变宽`, t`B 隐藏但占位`, t`删除 B 并重排`] as const;
  const [mode, setMode] = useState(0);

  return (
    <Panel
      title={t`9. 动态尺寸 / 隐藏 / 删除 / 重排`}
      width={1504}
      height={848}
      note={t`预期：每次点击同帧完成重排；visible = false 仍占位；删除后兄弟项递补到最前。`}
    >
      <hbox x={20} y={64} gap={20} alignItems="center">
        <Button
          sprite={{ ...BUTTON_SPRITE, targetWidth: 320, targetHeight: 56 }}
          text={t`下一状态：${dynamicLabels[(mode + 1) % dynamicLabels.length]}`}
          textStyle={BUTTON_TEXT_STYLE}
          onPress={() => setMode((value) => (value + 1) % dynamicLabels.length)}
        />
        <text {...TEXT.body} text={t`当前：${dynamicLabels[mode]}`} fillColor={COLOR.accent} />
      </hbox>
      ...
      <vbox x={20} y={440} gap={12}>
        <text {...TEXT.body} text={t`状态说明`} fillColor={COLOR.panelTitle} />
        <text {...TEXT.caption} text={t`· 初始布局：A、B、C 顺序排列，B 宽 150。`} />
        <text {...TEXT.caption} text={t`· B 变宽：B 宽度变为 260，C 相应后移。`} />
        <text {...TEXT.caption} text={t`· B 隐藏但占位：visible = false 不渲染，但布局位置保留。`} />
        <text {...TEXT.caption} text={t`· 删除 B 并重排：B 从树中移除，C 移动到 A 之前。`} />
      </vbox>
    </Panel>
  );
}
  ...
  return (
    <container>
      <SectionTabs
        ...
        onChange={setTab}
        options={[
          { value: 'basic', label: t`基础与嵌套` },
          { value: 'alignment', label: t`对齐交互` },
          { value: 'transform', label: t`变换与测量` },
          { value: 'dynamic', label: t`动态重排` },
        ]}
      />
      <container y={64}>
