...
export function InputPage() {
  const { t } = useLingui();
  const [controlledValue, setControlledValue] = useState(t`受控初始值`);
  const [controlledState, setControlledState] = useState<EditableState | null>(null);
  const [readOnly, setReadOnly] = useState(true);
  const [disabled, setDisabled] = useState(true);
  const [lastEvent, setLastEvent] = useState(t`等待输入事件`);
  ...
  return (
    <vbox gap={24}>
      <Panel
        ...
        width={1504}
        height={400}
        note={t`悬停和按住输入框应切换背景；聚焦后应显示 focused 背景。再验证焦点切换、末尾输入、Backspace 与中文输入法候选窗位置。`}
      >
        <vbox gap={18}>
          <text {...TEXT.caption} text={t`非受控输入（autoFocus）`} />
          <Input
          ...
          />
          <text {...TEXT.caption} text={t`第二个输入框（用于焦点切换）`} />
          <Input
            defaultValue={t`第二个输入框`}
            width={700}
            height={60}
            paddingX={16}
            textStyle={INPUT_TEXT_STYLE}
      ...
      <Panel
        ...
        width={1504}
        height={300}
        note={t`使用 Microsoft 拼音输入时，候选窗应跟随缩放、旋转后的文字末尾，并保持在 caret 附近。`}
      >
      ...
      <Panel
        ...
        width={1504}
        height={500}
        note={t`受控值应跟随 onChange 回写；只读可聚焦但不能编辑；点击禁用输入会清除当前焦点。`}
      >
        <hbox gap={48}>
          <vbox gap={18}>
            <text {...TEXT.caption} text={t`受控输入`} />
            <Input
            ...
            <Button
              ...
              onPress={() => setControlledValue(t`由外部状态设置`)}
            />
            <text
              {...TEXT.caption}
          ...
          <vbox gap={18}>
            <text {...TEXT.caption} text={t`readOnly：允许聚焦，不允许输入`} />
            <Input
              defaultValue={t`只读内容`}
              readOnly={readOnly}
              width={700}
            ...
            <Button
              sprite={{ ...BUTTON_SPRITE, targetWidth: 320, targetHeight: 52 }}
              text={readOnly ? t`切换为可编辑` : t`切换为只读`}
              textStyle={BUTTON_TEXT_STYLE}
              onPress={() => setReadOnly((current) => !current)}
            />
            <text {...TEXT.caption} text={t`disabled：点击后不获得焦点`} />
            <Input
              defaultValue={t`禁用内容`}
              disabled={disabled}
