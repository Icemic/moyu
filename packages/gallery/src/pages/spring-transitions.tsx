...
function UseSpringPanel() {
  ...
  return (
    <Panel title={t`useSpring 属性动画`} width={1504} height={320}>
...
function UseTransitionPanel() {
  ...
  return (
    <Panel
      title={t`useTransition 进出场`}
      width={1504}
      height={280}
      note={t`添加 / 移除 keyed 项目，from → enter → leave 自动补间。`}
    >
      <hbox gap={16}>
        <Button
...
function FadeHelpersPanel() {
  ...
  return (
    <Panel title={t`淡入淡出辅助`} width={1504} height={300}>
      <hbox y={16} gap={16}>
        <Button
          sprite={{ ...BUTTON_SPRITE, targetWidth: 180, targetHeight: 48 }}
          text={t`重播`}
      ...
      <clip x={20} y={100} width={1440} height={150}>
        <hbox gap={32}>
          ...
          >
            <text text={t`淡入 FADE IN`} fontSize={24} fillColor={COLOR.caption} x={40} y={30} />
          </animated.sprite>
          <animated.text text={t`淡出 FADE OUT`} fontSize={34} fillColor={COLOR.accent} opacity={fadeOutStyle.opacity} />
          <container>
            <text text={t`背景目标 BACKDROP`} fontSize={48} fillColor={ITEM_COLORS[3]} />
            <animated.backdrop
              width={230}
              height={130}
              x={100}
