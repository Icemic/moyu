...
function UseSpringPanel() {
  ...
  return (
    <Panel title="useSpring 属性动画" width={1504} height={280}>
      <clip x={20} y={56} width={1440} height={170}>
        <animated.container
          x={movement.x}
          y={40}
        ...
        <text {...TEXT.caption} text="点击 SPRING 切换目标状态" x={1180} y={70} />
      </clip>
      <text
        {...TEXT.body}
        text={`当前目标：x=${moved ? 620 : 80} · rotation=${moved ? 0.15 : -0.08} · scale=${moved ? 1.25 : 1} · opacity=${
          moved ? 0.65 : 1
        }`}
        x={20}
        y={240}
...
function UseTransitionPanel() {
  ...
  return (
    <Panel title="useTransition 进出场" width={1504} height={300} note="添加 / 移除 keyed 项目，from → enter → leave 自动补间。">
      <hbox x={20} y={60} gap={16}>
        <Button
      ...
      <hbox x={20} y={150} gap={28} alignItems="center">
        {transitions((style, item) => (
          <animated.container key={item} x={style.x} opacity={style.opacity} scale={style.scale}>
            <DemoChip label={`项目 ${item}`} width={120} height={64} color={ITEM_COLORS[(item - 1) % ITEM_COLORS.length]} />
          </animated.container>
        ))}
      </hbox>
    </Panel>
...
function FadeHelpersPanel() {
  const [fadeInStyle, fadeInApi, finishFadeIn] = useFadeIn(600, true);
  const [fadeOutStyle, fadeOutApi, finishFadeOut] = useFadeOut(600, true);
  ...
  const replayFades = () => {
    ...
    void fadeInApi.start({ to: { opacity: 1 }, pause: false, config: { duration: 600 } });
    void fadeOutApi.start({ to: { opacity: 0 }, pause: false, config: { duration: 600 } });
  };

  return (
    <Panel title="淡入淡出辅助" width={1504} height={268}>
      <hbox x={20} y={48} gap={16}>
        <Button
          sprite={{ ...BUTTON_SPRITE, targetWidth: 180, targetHeight: 46 }}
          text="重播"
          textStyle={BUTTON_TEXT_STYLE}
          onPress={replayFades}
        />
      ...
      <clip x={20} y={100} width={1440} height={150}>
        <text text="背景目标 BACKDROP" fontSize={48} fillColor={ITEM_COLORS[3]} x={650} y={50} />
        <animated.backdrop
          width={440}
          height={130}
          x={610}
