import { Button, ScrollView, getStageSize, useScrollView } from '@momoyu-ink/kit';
...
const PAGES: readonly GalleryPageDefinition[] = [
  { key: 'primitives', title: '基础组件', description: 'Sprite、Text、Clip、Animation 与通用节点属性。', component: PrimitivesPage },
  { key: 'controls', title: '封装组件', description: 'Button、Checkbox、Select、Slider 与 ScrollView。', component: ControlsPage },
  { key: 'filters', title: 'Filter 滤镜', description: '对节点自身子树的离屏渲染结果应用滤镜链。', component: FiltersPage },
  { key: 'backdrops', title: 'Backdrop 背景滤镜', description: '处理节点背后已经绘制的画面，之后绘制的内容保持清晰。', component: BackdropsPage },
  { key: 'spring-transitions', title: 'Spring 动画', description: 'useSpring 属性动画与 useTransition 进出场动画。', component: SpringTransitionsPage },
  { key: 'shader-transitions', title: 'Shader 转场', description: '双通道画面的 GPU 转场状态机。', component: ShaderTransitionsPage },
  { key: 'raw-shaders', title: '自定义 Shader', description: 'Raw WGSL、参数槽与时间控制。', component: RawShadersPage },
    ...
    <container label="Moyu Gallery" x={stageSize.width / 2} y={stageSize.height / 2} scale={scale}>
      ...
      <container x={-STAGE_WIDTH / 2} y={-STAGE_HEIGHT / 2}>
              ...
              >
                {PAGES.map((page, index) => (
                  <NavItem
                    ...
                    onPress={() => setCurrentPageKey(page.key)}
                  />
                ))}
              </ScrollView>
            </container>
        ...
        <container x={372} y={40}>
          ...
          <sprite src={PIXEL_SPRITE} tint={COLOR.panelTint} targetWidth={CONTENT_WIDTH} targetHeight={2} y={86} />
        </container>
