...
// (SelectListProps / SliderTrackProps / SliderThumbProps lack NodeProps.tint).
...
function dropdownTriggerSprite(fill) {
  ...
  strokeSegment(c, u(51), u(26), u(56), u(33), u(4), WHITE);
  strokeSegment(c, u(56), u(33), u(61), u(26), u(4), WHITE);
  return c;
}
...
function sliderTrackSprite() {
  const s = SUPERSAMPLE;
  const w = 64 * s;
  const h = 40 * s;
  const c = createCanvas(w, h);
  fillRoundRect(c, 0, h / 2 - 6 * s, w, h / 2 + 6 * s, 6 * s, GRAY(96));
  strokeRoundRect(c, s, h / 2 - 5 * s, w - s, h / 2 + 5 * s, 5 * s, 2 * s, WHITE);
  return c;
}

function sliderHandleSprite() {
  const s = SUPERSAMPLE;
  const w = 40 * s;
  const c = createCanvas(w, w);
  fillCircle(c, w / 2, w / 2, 15 * s, GRAY(210));
...
function sampleImage() {
  ...
  strokeRoundRect(c, 1, 1, w - 1, h - 1, 8, 3, [255, 255, 255, 200]);
  return c;
}
...
save(sliderTrackSprite(), 'slider_track.png', { tint: CONTROL_TINT });
save(sliderTrackSprite(), 'slider_track_hover.png', { tint: CONTROL_TINT });
save(sliderTrackSprite(), 'slider_track_press.png', { tint: CONTROL_TINT });
save(sliderHandleSprite(), 'slider_handle.png', { tint: CONTROL_TINT });
save(sliderHandleSprite(), 'slider_handle_hover.png', { tint: CONTROL_TINT });
save(sliderHandleSprite(), 'slider_handle_press.png', { tint: CONTROL_TINT });
