...
包含节点自身和子树视觉变换结果的轴对齐包围盒。它用于渲染可见性、命中、裁剪和离屏效果，不作为自动布局占位尺寸。
...
`Shader` 和 `ShaderSlot` 当前依赖父节点尺寸，因此不能作为正常的 `VBox`/`HBox` 直接子项。它们返回“不参与父级测量”，行为如下：

- 不计入自动尺寸、项目数、gap 或 `space-between`；
- 采用零占位；
- 引擎记录警告；
- 如需使用，应放入具有确定尺寸或内容测量语义的普通包装节点内。
...
需要裁剪时应显式使用 `Clip`。Clip 使用自身显式矩形作为布局尺寸，不因子树视觉溢出自动扩大占位。
...
- `Shader` / `ShaderSlot` 直接出现在单轴布局中时，记录警告并按零占位处理；
- 内容溢出不是错误，不记录警告。
...
5. **系统回归**：content bounds、命中、Clip、Filter、Backdrop、Shader、可见性和渲染命令顺序。
...
- 让 Shader / ShaderSlot 获得独立内容尺寸，支持作为正常单轴布局子项；
- 检查并修复 Clip 的局部起点和 global transform 边界计算；
